...
type Service struct {
	...
	locks            *dossierLocks
	candidateDigests *candidateDigestCache
	now              func() time.Time
}
...
func New(store *repository.Store) *Service {
	return &Service{
		...
		candidateDigests: newCandidateDigestCache(),
		now:              time.Now,
	}
}
...
func (s *Service) Get(ctx context.Context, id string) (Detail, error) {
	...
	if d.Status == domain.StatusConfirmation {
		digest, candidateErr := s.candidateDigests.digest(d.DossierID, func() (string, error) {
			text, err := d.CandidateText()
			if err != nil {
				return "", err
			}
			return domain.Digest(text), nil
		})
		if candidateErr == nil {
			detail.CandidateSHA256 = digest
		}
	} else if text, candidateErr := d.CandidateText(); candidateErr == nil {
