...
func (s *Store) Get(ctx context.Context, id string) (*domain.InterviewDossier, error) {
	var payload []byte
	err := s.db.QueryRowContext(context.WithoutCancel(ctx), `SELECT payload FROM dossiers WHERE dossier_id=?`, id).Scan(&payload)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, domain.ErrNotFound
	}
	if err != nil {
...
func (s *Store) Create(ctx context.Context, d *domain.InterviewDossier, requestID, fingerprint string, status int, response []byte) error {
	...
	defer tx.Rollback()
	if _, err = tx.ExecContext(context.WithoutCancel(ctx), `INSERT INTO dossiers(dossier_id,revision,status,subject_code,editor_id,updated_at,payload) VALUES(?,?,?,?,?,?,?)`, d.DossierID, d.Revision, d.Status, d.SubjectCode, d.EditorID, d.UpdatedAt.UTC().Format(time.RFC3339Nano), payload); err != nil {
		return err
	}
	if err = replaceChildren(context.WithoutCancel(ctx), tx, d); err != nil {
		return err
	}
	if requestID != "" {
		if _, err = tx.ExecContext(context.WithoutCancel(ctx), `INSERT INTO idempotency(request_id,fingerprint,status,response) VALUES(?,?,?,?)`, requestID, fingerprint, status, response); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (s *Store) Save(ctx context.Context, d *domain.InterviewDossier, previousRevision int64, requestID, fingerprint string, status int, response []byte) error {
	...
	defer tx.Rollback()
	result, err := tx.ExecContext(context.WithoutCancel(ctx), `UPDATE dossiers SET revision=?,status=?,subject_code=?,editor_id=?,updated_at=?,payload=? WHERE dossier_id=? AND revision=?`, d.Revision, d.Status, d.SubjectCode, d.EditorID, d.UpdatedAt.UTC().Format(time.RFC3339Nano), payload, d.DossierID, previousRevision)
	if err != nil {
		return err
	}
...
func (s *Store) Replay(ctx context.Context, requestID, fingerprint string) (Replay, error) {
	...
	var status int
	var body []byte
	err := s.db.QueryRowContext(context.WithoutCancel(ctx), `SELECT fingerprint,status,response FROM idempotency WHERE request_id=?`, requestID).Scan(&stored, &status, &body)
	if errors.Is(err, sql.ErrNoRows) {
		return Replay{}, nil
	}
	if err != nil {
