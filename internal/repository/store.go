...
type Store struct {
	...
	dossierCache map[string]*domain.InterviewDossier
}
...
func Open(path string) (*Store, error) {
	...
	s := &Store{db: db, dossierCache: map[string]*domain.InterviewDossier{}}
...
func (s *Store) Get(ctx context.Context, id string) (*domain.InterviewDossier, error) {
	...
	s.cacheMu.Lock()
	if cached = s.dossierCache[id]; cached == nil {
		s.dossierCache[id] = d
	} else {
		d = cached
	}
...
func (s *Store) Create(ctx context.Context, d *domain.InterviewDossier, requestID, fingerprint string, status int, response []byte) error {
	...
	s.cacheMu.Lock()
	delete(s.dossierCache, d.DossierID)
	s.cacheMu.Unlock()
	return nil
}
